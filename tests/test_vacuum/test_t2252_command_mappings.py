...
def test_t2252_mode_command_values(mock_t2252_robovac) -> None:
    ...
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "auto") == "Auto"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "small_room")
        == "SmallRoom"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "spot") == "Spot"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "edge") == "Edge"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "nosweep")
        == "Nosweep"
    )

    # Unknown returns as-is
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "unknown")
        == "unknown"
    )
...
def test_t2252_mode_case_insensitive(mock_t2252_robovac) -> None:
    ...
    assert (
        mock_t2252_robovac.getRoboVacHumanReadableValue(RobovacCommand.MODE, "auto")
        == "Auto"
    )
    assert (
        mock_t2252_robovac.getRoboVacHumanReadableValue(RobovacCommand.MODE, "Auto")
...
def test_t2252_fan_speed_command_values(mock_t2252_robovac) -> None:
    ...
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "standard")
        == "Standard"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "turbo")
        == "Turbo"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "max")
        == "Max"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "boost_iq")
        == "Boost_IQ"
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "unknown")
        == "unknown"
    )
...
def test_t2252_error_code_mapping(mock_t2252_robovac) -> None:
    ...
    assert (
        mock_t2252_robovac.getRoboVacHumanReadableValue(RobovacCommand.ERROR, "0")
        == "No error"
    )
...
def test_t2252_status_human_readable(mock_t2252_robovac) -> None:
    ...
    """
    assert (
        mock_t2252_robovac.getRoboVacHumanReadableValue(
...
def test_t2252_start_pause_values(mock_t2252_robovac) -> None:
    ...
    """
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.START_PAUSE, "start")
        is True
    )
    assert (
        mock_t2252_robovac.getRoboVacCommandValue(RobovacCommand.START_PAUSE, "pause")
        is False
    )
