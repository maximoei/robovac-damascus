...
def test_t2118_mode_command_values(mock_t2118_robovac) -> None:
    ...
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "auto") == "Auto"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "small_room")
        == "SmallRoom"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "spot") == "Spot"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "edge") == "Edge"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "nosweep")
        == "Nosweep"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.MODE, "unknown")
        == "unknown"
    )
...
def test_t2118_fan_speed_command_values(mock_t2118_robovac) -> None:
    ...
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(
            RobovacCommand.FAN_SPEED, "no_suction"
        )
        == "No_suction"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "standard")
        == "Standard"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "boost_iq")
        == "Boost_IQ"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "max")
        == "Max"
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.FAN_SPEED, "unknown")
        == "unknown"
    )
...
def test_t2118_status_human_readable(mock_t2118_robovac) -> None:
    ...
    """
    assert (
        mock_t2118_robovac.getRoboVacHumanReadableValue(
...
def test_t2118_error_human_readable(mock_t2118_robovac) -> None:
    ...
    """
    assert (
        mock_t2118_robovac.getRoboVacHumanReadableValue(RobovacCommand.ERROR, "0")
        == "No error"
    )
...
def test_t2118_mode_human_readable_nosweep(mock_t2118_robovac) -> None:
    ...
    """
    assert (
        mock_t2118_robovac.getRoboVacHumanReadableValue(RobovacCommand.MODE, "Nosweep")
        == "Nosweep"
    )
...
def test_t2118_start_pause_values(mock_t2118_robovac) -> None:
    ...
    """
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.START_PAUSE, "start")
        is True
    )
    assert (
        mock_t2118_robovac.getRoboVacCommandValue(RobovacCommand.START_PAUSE, "pause")
        is False
    )
